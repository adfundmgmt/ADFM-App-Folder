...
def _load():
    ...
    warns = json.loads(wpath.read_text(encoding="utf-8")) if wpath.exists() else {}
...
with st.sidebar:
    ...
    st.caption("Data: FRED · OECD · BIS · Eurostat · e-Stat · ONS · CFTC · Yahoo. "
               "See docs/CURRENCY_TENSION_DATA_SOURCES.md.")
...
with t3:
    st.dataframe(overlays.set_index("ccy"), width="stretch")
...
with t4:
    ...
    st.write("**Pillars:**")
    st.dataframe(pillars.set_index("ccy").loc[[ccy]], width="stretch")
    if ccy in warns:
        st.write("**Warnings:**")
        for n in warns[ccy]:
            st.markdown(f"- {n}")
