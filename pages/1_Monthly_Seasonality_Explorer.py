...
def fetch_prices(symbol, start, end):
    ...
    return _yf_download(symbol, start, end)
...
def intra_month_halves(prices: pd.Series):
...
def seasonal_stats(prices, start_year, end_year):
    ...
    halves = intra_month_halves(prices)
...
def plot_seasonality(stats, title):
    ...
    df = stats.dropna().copy()

    labels = df["label"].tolist()

    mean_h1 = df["mean_h1"].values
    mean_h2 = df["mean_h2"].values
    ...
    ax.bar(x, mean_h1, color="#62c38e")

    ax.bar(x, mean_h2, bottom=mean_h1, hatch="///", color="#62c38e")

    yerr = np.vstack([totals - p05, p95 - totals])

    ax.errorbar(x, totals, yerr=yerr, fmt="none", ecolor="gray")
    ...
    ax2.scatter(x, hit, color="black", marker="D")
    ...
    table = ax_tbl.table(
        cellText=[
            [f"{v:+.1f}%" for v in mean_h1],
            [f"{v:+.1f}%" for v in mean_h2],
            [f"{v:+.1f}%" for v in totals]
        ],
...
def build_month_paths(prices, month, start_year, end_year):
