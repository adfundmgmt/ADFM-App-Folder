...
st.title("S&P Cyclicals Relative to Defensives — Equal‑Weight (Robust Membership)")
...
# Only keep ETFs with full data for entire window (Bloomberg style)
keep = prices.notna().all(axis=0)
clean_prices = prices.loc[:, keep]

cyc_live = [t for t in CYCLICALS  if t in clean_prices]
def_live = [t for t in DEFENSIVES if t in clean_prices]

st.write(f"**Cyclicals in basket:** {cyc_live}")
st.write(f"**Defensives in basket:** {def_live}")

if len(cyc_live) < 2 or len(def_live) < 2:
    st.error("Not enough ETFs with full data for selected window. Try a shorter window or check ticker universe.")
else:
    rebased = clean_prices / clean_prices.iloc[0] * 100
