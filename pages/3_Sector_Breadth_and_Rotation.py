...
highlight_sector = st.selectbox(
    "Highlight sector (for overlay):", SECTOR_LIST, index=0, key="sector_highlight"
)
...
with col1:
    ...
    fig_rot.update_layout(
        title="Sector Rotation Quadrant",
...
with col2:
    # --- Sector Relative Strength Overlay ---
...
# Calculate 1W, 1M, 3M returns as percent
for k, n in zip(["1W", "1M", "3M"], [5,21,63]):
    perf[k] = data[[SECTOR_ETFS[x] for x in SECTOR_LIST]].pct_change(n).iloc[-1]*100
# Calculate YTD return (robust method)
start_of_year = data.index[data.index.get_loc(str(datetime.today().year) + "-01-02", method='bfill')]
perf["YTD"] = (data[[SECTOR_ETFS[x] for x in SECTOR_LIST]].iloc[-1].values /
    data[[SECTOR_ETFS[x] for x in SECTOR_LIST]].loc[start_of_year].values - 1) * 100

ma_windows = [20, 50, 200]
for win in ma_windows:
    perf[f"Above {win}D"] = [
        100.0 if data[SECTOR_ETFS[sector]].iloc[-1] > data[SECTOR_ETFS[sector]].rolling(win).mean().iloc[-1] else 0.0
        for sector in SECTOR_LIST
    ]

# --- Ensure table is float, fillna(0) for coloring ---
for col in ["1W", "1M", "3M", "YTD"]:
    perf[col] = pd.to_numeric(perf[col], errors='coerce').fillna(0)
for win in ma_windows:
    perf[f"Above {win}D"] = pd.to_numeric(perf[f"Above {win}D"], errors='coerce').fillna(0)

# --- Table Styling ---
st.dataframe(
    perf.style
        .format("{:.2f}")
        .background_gradient(cmap="RdYlGn", subset=["1W", "1M", "3M", "YTD"]),
