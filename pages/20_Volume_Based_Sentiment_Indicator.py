...
COLORS = {
    ...
    "vol_stress": "rgba(239, 83, 80, 0.74)",
    "vol_quiet": "rgba(251, 140, 0, 0.62)",
    "vol_ma": "#2962FF",
    "zscore": "#111827",
    "stress_line": "rgba(239, 83, 80, 0.92)",
    "quiet_line": "rgba(251, 140, 0, 0.92)",
    "zero_line": "rgba(120, 120, 120, 0.55)",
    "stress_fill": "rgba(239, 83, 80, 0.055)",
    "quiet_fill": "rgba(251, 140, 0, 0.030)",
...
def _request_text(url, timeout=20, retries=3, sleep_s=1.0):
    last_err = None
...
def fetch_from_stooq(symbol, start_date):
...
def fetch_from_yahoo_chart_api(symbol, start_date, end_date):
...
def fetch_from_yfinance_download(symbol, start_date, end_date):
    df = yf.download(
        symbol,
...
def sparse_signal_points(df, signal_col, gap_days=18):
