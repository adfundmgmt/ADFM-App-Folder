...
# ------ Date ranges ------
now = datetime.today()
hist_start = now - timedelta(days=365*15)  # 15-year history for MAs
if span_key == "YTD":
...
all_tickers = set(CYCLICALS + DEFENSIVES + [t for p in PRESETS for t in p] + ['SPY'])
...
def compute_ratio(s1, s2, scale=100):
    return (s1 / s2) * scale
...
def make_fig(ratio, title, ylab):
    ...
    # Top: Ratio & MAs
    ax1.plot(view.index, view, color='black', linewidth=1, label=title)
    ax1.plot(ma50.index, ma50, color='blue', linewidth=1, label='50-DMA')
    ax1.plot(ma200.index, ma200, color='red', linewidth=1, label='200-DMA')
    ...
    # Bottom: RSI
    ax2.plot(rsi.index, rsi, color='black', linewidth=1)
...
ratio_cd = compute_ratio(cumrets[CYCLICALS].mean(axis=1),
                         cumrets[DEFENSIVES].mean(axis=1))
fig_cd = make_fig(ratio_cd, 'Cyclicals / Defensives (Eq-Wt)', 'Ratio')
st.pyplot(fig_cd, use_container_width=True)

# 2. SPY cumulative return
cum_spy = compute_cumrets(closes[['SPY']])['SPY']
fig_spy = make_fig(cum_spy, 'SPY Cumulative Return', 'Cumulative Return')
st.pyplot(fig_spy, use_container_width=True)

st.markdown('---')

# 3. Preset pairs
for t1, t2 in PRESETS:
    ratio = compute_ratio(cumrets[t1], cumrets[t2], scale=1)
    fig = make_fig(ratio, f'{t1}/{t2}', f'{t1}/{t2}')
...
if custom_t1 and custom_t2:
    if custom_t1 in cumrets and custom_t2 in cumrets:
        ...
        fig_custom = make_fig(ratio_custom,
