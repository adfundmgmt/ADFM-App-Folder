Got it. The root cause is still the windowing. We’ll clamp by dates using the **last available bar** as `end`, fetch **exactly** that window from Yahoo at the chosen bar size, compute indicators with a buffer, then hard-limit the x-axis on **all** subplot axes. No resampling tricks, no row counts.

Below is a clean, working build. Paste it as a full file.
...
* We fetch **weekly or monthly bars directly from Yahoo** for the selected window, so the DataFrame never contains 2011 for a 3y view.
* Indicators are computed on those bars with a left-hand buffer, then we trim to the exact window.
* We force the x-range on **every subplot x-axis** so nothing slips the clamp when you change interval.
