...
st.sidebar.markdown(
    "Visualize OHLC candlesticks with MAs, RSI, and MACD using Yahoo Finance data."
)
...
def fetch_daily(tkr: str, start_dt: pd.Timestamp, end_dt: pd.Timestamp) -> pd.DataFrame:
    ...
    # remove weekends if any sneaked in
...
# left buffer for indicators so edges are smooth
buffer_days = 260
...
for w, color in zip((8, 20, 50, 100, 200), ("green", "purple", "blue", "orange", "gray")):
    fig.add_trace(
        go.Scatter(x=plot_df.index, y=plot_df[f"MA{w}"], mode="lines", line=dict(color=color, width=1.2), name=f"MA{w}"),
        row=1,
        col=1,
    )
...
fig.add_trace(
    go.Bar(x=plot_df.index, y=plot_df["Volume"], marker_color=vol_colors, opacity=0.7, name="Volume"),
    row=2,
    col=1,
)
...
# RSI
fig.add_trace(
    go.Scatter(x=plot_df.index, y=plot_df["RSI14"], mode="lines", line=dict(width=1.5, color="purple"), name="RSI (14)"),
    row=3,
    col=1,
)
...
# X axis clamp for all subplots, hide weekends
