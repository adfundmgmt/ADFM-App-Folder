...
# ===================== Settings =====================
MAX_YEARS = 12  # cap for "max" to avoid heavy charts. Change as you like.
...
def get_daily(tkr: str) -> pd.DataFrame:
    ...
    # drop weekends for uniform spacing
...
if off is None:
    # "max" requested. Apply MAX_YEARS cap from end.
    start = max(df.index.min(), end - pd.DateOffset(years=MAX_YEARS))
else:
    start = end - off

# add indicators on full df so MAs are continuous even at left edge
