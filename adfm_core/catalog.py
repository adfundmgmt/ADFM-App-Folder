...
TOOL_CATALOG: Final[tuple[ToolDefinition, ...]] = (
    ...
    ToolDefinition(8, "Factor Momentum Leadership", "8_Factor_Momentum_Leadership.py", "Equity Leadership", "Ranks factor momentum to highlight which styles are leading, fading, or inflecting.", "Yahoo Finance ETF prices"),
    ...
    ToolDefinition(11, "Ratio Charts", "11_Ratio_Charts.py", "Technical Confirmation", "Uses relative-strength ratios to compare assets, sectors, credit, factors, and risk appetite proxies.", "Yahoo Finance adjusted close history"),
    ...
    ToolDefinition(17, "CFTC Positioning Monitor", "17_CFTC_Positioning_Monitor.py", "Positioning + Flows", "Scans CFTC financial and physical futures for crowded longs, crowded shorts, and weekly positioning shifts, with cohort history and price overlays.", "CFTC Public Reporting Environment TFF and Disaggregated futures-only reports; Yahoo Finance continuous-futures price proxies"),
    ToolDefinition(18, "Market Stress Composite", "18_Market_Stress_Composite.py", "Risk + Execution", "Builds a cross-asset stress score across equities, credit, commodities, FX, rates, breadth, and dispersion.", "Yahoo Finance; local last-good cache on provider failure"),
    ToolDefinition(19, "Event Risk + Catalyst Calendar", "19_Event_Risk_Catalyst_Calendar.py", "Risk + Execution", "Maps upcoming macro catalysts, options windows, Treasury supply, earnings season, and custom event risks.", "Yahoo Finance market proxies; configured calendar data"),
    ToolDefinition(20, "Hedge Timer", "20_Hedge_Timer.py", "Risk + Execution", "Provides tactical timing cues for adding, holding, reducing, or rolling portfolio hedges.", "Yahoo Finance; FRED regime inputs"),
    ToolDefinition(21, "Position Sizing Lab", "21_Position_Sizing_Lab.py", "Risk + Execution", "Runs an interactive bankroll simulation using real historical holding-period outcomes, then pressure-tests conviction-based exposure against volatility, invalidation, event, tail, and liquidity risk.", "Yahoo Finance adjusted OHLCV, earnings dates, and liquid cross-asset proxies"),
    ToolDefinition(22, "Market Memory Explorer", "22_Market_Memory_Explorer.py", "Historical Context", "Surfaces historical analogs to contextualize the current tape against prior return paths and regimes.", "Yahoo Finance market history"),
    ToolDefinition(23, "Monthly Seasonality Explorer", "23_Monthly_Seasonality_Explorer.py", "Historical Context", "Shows recurring monthly return and volatility patterns by asset, index, sector, or commodity.", "Yahoo Finance; FRED for selected series and regime tags"),
)
