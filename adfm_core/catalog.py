...
TOOL_CATALOG: Final[tuple[ToolDefinition, ...]] = (
    ...
    ToolDefinition(2, "Global Bond Monitor", "2_Global_Macro_Regime.py", "Macro Regime", "Compares U.S. Treasury yields, real rates, inflation compensation, credit spreads and monthly global sovereign yields with dated basis-point changes.", "Federal Reserve and ICE BofA via FRED; OECD monthly sovereign rates via FRED"),
    ...
    ToolDefinition(21, "Hedge Timing", "21_Hedge_Timer.py", "Risk + Execution", "Runs a high-recall Hedge Watch for the S&P 500 and Nasdaq-100, calibrated to capture 10%+ local-peak drawdowns since 2020 before applying a separate confirmation and fresh-short gate.", "Yahoo Finance adjusted closes for S&P 500, Nasdaq-100, SPY, RSP, IWM, HYG, LQD, all 11 S&P 500 sector ETFs, VIX, VIX9D, VIX3M, and VVIX"),
...
SIDEBAR_GUIDES: Final[dict[str, SidebarGuide]] = {
    ...
    "2_Global_Macro_Regime.py": SidebarGuide(("Select the Treasury, real-rate, sovereign or credit market.", "Compare dated levels and basis-point moves in the table.", "Select an instrument to inspect its yield history."), "Global sovereign yields are monthly averages; U.S. series are daily observations."),
    ...
    "20_Catalyst_Calendar.py": SidebarGuide(("Choose the event horizon and scan the dated catalyst sequence.", "Prioritize events by timing, risk score, and the market setup going into them.", "Add mandate-specific events with the custom-event template when needed."), "Confirm agency schedules before trading directly around a release; recurring market dates can be rule-based."),
    "21_Hedge_Timer.py": SidebarGuide(("Start with Hedge Watch for both SPX and NDX; it is the high-recall warning layer before short-entry gates.", "Use confirmation, RSI, and the 63-session drawdown gates to decide whether a fresh directional short is allowed.", "Audit every 10%+ drawdown from a rolling local high since 2020, then compare lead time and false warnings."), "Calibration gives historical drawdown recall priority over false positives. Treat the output as timing evidence within a hedge plan, not as a standalone trade instruction."),
    "22_Position_Sizing_Lab.py": SidebarGuide(("Define direction, conviction, holding period, loss budget, and liquidity constraints.", "Review the volatility, invalidation, event, tail, and liquidity caps.", "Run multiple historical paths and size from the binding risk cap, not the best simulation outcome.")),
