...
def _inject_page_style() -> None:
    ...
    st.markdown(
        ...
        <style>
        .event-study-status {
            display:flex; flex-wrap:wrap; gap:.50rem 1.10rem; align-items:center;
            border-top:1px solid #d7d7d7; border-bottom:1px solid #d7d7d7;
            margin:.20rem 0 .55rem; padding:.55rem 0; color:#222222;
        ...
        .event-study-table-title {
            margin:1.35rem 0 .72rem; color:#000; font-family:Georgia,"Times New Roman",serif;
            font-size:1.28rem; font-weight:700; letter-spacing:-.018em; line-height:1.20;
        }
        .event-study-table-wrap {
            width:100%; overflow-x:auto; -webkit-overflow-scrolling:touch;
            margin:.15rem 0 .58rem; border:1px solid #aeb7bd; background:#fff;
        }
        ...
        table.event-study-table thead th {
            background:#357f8d; color:#fff; font-weight:800; letter-spacing:.01em;
        }
        ...
        table.event-study-table tbody td.pos { background:#dce9e1; color:#111; }
        table.event-study-table tbody td.neg { background:#edc9cd; color:#111; }
        ...
        @media (max-width:800px) {
            .event-study-status { gap:.38rem .72rem; font-size:.73rem; }
            .event-study-table-title { font-size:1.12rem; }
...
def _render_sidebar() -> None:
    with st.sidebar:
        ...
        st.markdown(
            """
            **Purpose:** Test what historically happened after a repeatable commodity price event.

            Pick a Yahoo Finance futures contract, define the event, and the page marks every qualifying historical occurrence on the price chart. The table then measures the distribution of forward returns and drawdowns from 1 day through 12 months.

            **How to read it:**
            - **Average / Median:** central forward return after prior signals.
            - **Best / Worst:** historical outcome range.
            - **% Positive:** share of completed observations above the signal close.
            - **DD From Signal:** decline from the signal close during the horizon.
            - **Peak-to-Trough DD:** worst decline from any interim peak after the signal.
            - **Sample:** matured observations available for that horizon.

            **Use it for:** checking whether an apparent tape extreme has historically shown continuation, mean reversion, or poor asymmetry.
            """
        )
        st.caption(
            "Yahoo futures histories are provider-supplied continuous series. Contract-roll methodology can affect historical price paths."
        )
...
def build_event_observations(
    ...
    store: Dict[str, Dict[str, List[float]]] = {
        label: {"return": [], "signal_dd": [], "path_dd": []}
    ...
    arrays = {
        label: {metric: np.asarray(values, dtype=float) for metric, values in metrics.items()}
...
def make_price_chart(
    ...
    if len(event_dates):
        ...
        for event_date in event_dates:
            ...
            hover_text.append(
                f"{event_date:%b %d, %Y}<br>Price: {event_prices.loc[event_date]:,.2f}<br>"
                f"Signal: {_format_signal_value(value, signal_value_kind)}"
            )
        figure.add_trace(
            go.Scatter(
                x=event_dates,
                ...
                mode="markers",
                marker={"color": "#e52822", "size": 7, "line": {"color": "#ffffff", "width": 0.65}},
                text=hover_text,
    ...
    figure.update_xaxes(
        showgrid=False, showline=True, linecolor="#8d8d8d", linewidth=1,
        ticks="outside", tickcolor="#8d8d8d", tickformat="%Y", fixedrange=False,
    )
    figure.update_yaxes(
        showgrid=True, gridcolor="#e3e8ec", gridwidth=1, zeroline=False,
        showline=False, tickformat=",.2f", title=None, fixedrange=False,
    )
    return figure
...
def summary_table_html(summary: pd.DataFrame) -> str:
    ...
    colgroup = "<col class='metric-col'>" + "".join("<col class='horizon-col'>" for _ in columns)
    body_rows = []
    for metric, row in summary.iterrows():
        cells = "".join(
            f"<td class='{_cell_class(metric, row[column])}'>{_format_summary_value(metric, row[column])}</td>"
            for column in columns
        )
        body_rows.append(f"<tr><td class='metric'>{escape(metric)}</td>{cells}</tr>")
    return (
...
def render_commodity_event_study() -> None:
    ...
    if signal_type == "Return threshold":
        ...
        with param_columns[2]:
            threshold = st.number_input(
                "Threshold (%)", min_value=1.0, max_value=300.0,
        ...
        with param_columns[2]:
            threshold = st.number_input(
                "RSI threshold", min_value=1.0, max_value=99.0,
                value=70.0 if direction == "Overbought" else 30.0,
                step=1.0, format="%.2f"
            )
        ...
        with param_columns[2]:
            threshold = st.number_input(
                "Stretch (%)", min_value=1.0, max_value=200.0,
    ...
    st.markdown(
        (
            ...
            f"<span><strong>Signal</strong> {escape(signal_label)}</span>"
    ...
    figure = make_price_chart(chart_close, study_events, signal_metric, signal_label, signal_kind)
    st.plotly_chart(figure, width="stretch", config={"displayModeBar": False})

    st.markdown(
        f"<div class='event-study-table-title'>{escape(contract_name)} Forward Performance After Event</div>",
        unsafe_allow_html=True,
    )
