...
PILLAR_LABEL = {
    "A_growth": "Growth (A)",
    "B_inflation": "Inflation (B)",
    "C_external": "External (C)",
    "D_fiscal": "Real 10Y (D)",
    "E_policy": "Policy (E)",
    "F_carry": "Carry (F)",
    "G_valuation": "Valuation (G)",
}
...
def tension_map_fig(
    ...
    if history is not None and trail_months and len(history):
        ...
        me = (
            h.assign(me=h.date + pd.offsets.MonthEnd(0))
            .sort_values("date")
            .groupby(["ccy", "me"])
            .tail(1)
        )
        for ccy in d.index:
            ...
            if len(t) >= 1:
                pts = list(zip(t[xcol], t[ycol], strict=False)) + [
                    (d.loc[ccy, xcol], d.loc[ccy, ycol])
                ]
                if len(pts) >= 2:
                    trails[ccy] = pts
    ...
    xlo, xhi = _bounds(pd.Series(list(xs) + tx))
    ylo, yhi = _bounds(pd.Series(list(ys) + ty))
    ...
    ax.add_patch(
        plt.Rectangle(
            (0, ylo), xhi, -ylo, color=_QUAD_TINT[("+", "-")], alpha=0.5, zorder=0
        )
    )
    ax.add_patch(
        plt.Rectangle(
            (xlo, 0), -xlo, yhi, color=_QUAD_TINT[("-", "+")], alpha=0.5, zorder=0
        )
    )
    ...
    for (sx, sy), label in _QUAD.items():
        ax.text(
            (xhi if sx == "+" else xlo) * 1.0
            - (0.02 * (xhi - xlo) * (1 if sx == "+" else -1)),
            (yhi if sy == "+" else ylo)
            - (0.03 * (yhi - ylo) * (1 if sy == "+" else -1)),
            label,
            color=_MUTE,
            fontsize=9.5,
    ...
    _HZ_LABEL = {
        "struct": "Structural · ~10y",
        "regime": "Regime · ~2y",
        "secular": "Secular · ~15y",
    }
    hlabel = _HZ_LABEL.get(horizon, horizon)
    ax.set_xlabel(
        "←  Deteriorating        Fundamental trajectory        Improving  →",
        fontsize=9.5,
        labelpad=8,
    )
    ax.set_ylabel(
        "←  Cheap        Valuation & policy stretch        Maxed-out  →",
        fontsize=9.5,
        labelpad=8,
    )
    suffix = f"   ·   as of {asof_label}" if asof_label else ""
    ax.set_title(
        f"Currency Tension Map   ·   {hlabel}{suffix}",
        fontsize=13.5,
        pad=14,
        fontweight="bold",
    )
...
def carry_heatmap_fig(grid, title="2Y Real Carry  ·  Base − Quote (%)"):
    ...
    ax.set_xticklabels(ccys, fontsize=10)
    ax.set_yticklabels(ccys, fontsize=10)
    ...
    for i in range(n):
        ax.add_patch(plt.Rectangle((i - 0.5, i - 0.5), 1, 1, color=_PANEL, zorder=2))
        for j in range(n):
            if i != j:
                v = vals[i, j]
                ax.text(
                    ...
                    va="center",
                    fontsize=8.5,
                    color=("white" if abs(v) > m * 0.45 else _FG),
                    zorder=3,
                )
...
def pillar_heatmap_fig(pillars, tm=None, horizon="struct"):
    ...
    if tm is not None:
        t = tm.set_index("ccy")
        order = (
            t[f"axis1_fundamental_{horizon}"]
            .reindex(p.index)
    ...
    ax.set_title("Pillar Scores", color=_FG, fontsize=13, pad=26, fontweight="bold")
...
def positioning_fig(pos):
    ...
    _style(ax)
    zcols = (
        d[["lev_z", "am_z", "lev_z_13w", "am_z_13w"]]
        if "am_z_13w" in d.columns
        else d[["lev_z", "am_z"]]
    )
    ...
    def _cohort(y, now, was, color, pct):
        ...
        ax.scatter(now, y, s=150, color=color, zorder=4, edgecolors=_BG, linewidths=1.2)
        if pd.notna(pct):
            ax.text(
                lim * 1.03,
                y,
    ...
    for i, r in d.iterrows():
        if i:  # soft dotted separator between currency bands
            ax.axhline(
                i - 0.5, color=_MUTE, lw=0.8, ls=(0, (1, 3)), alpha=0.5, zorder=1
            )
        _cohort(i + OFF, r.lev_z, getattr(r, "lev_z_13w", np.nan), _BLUE, r.lev_pct_oi)
        _cohort(i - OFF, r.am_z, getattr(r, "am_z_13w", np.nan), _TEAL, r.am_pct_oi)

    ax.set_yticks(range(n))
    ax.set_yticklabels(d.ccy, fontsize=11)
    ax.set_xlim(-lim, lim * 1.24)
    ax.set_ylim(-0.6, n - 0.4)
    ax.set_xlabel(
        "Net position, % of open interest — z vs own 10y history (long the currency →)",
        fontsize=9.5,
        labelpad=8,
    )
    ax.set_title(
        "Speculative Positioning   ·   CFTC TFF, futures + options",
        fontsize=13,
        pad=12,
        fontweight="bold",
    )
    handles = [
        ...
        Line2D(
            ...
            label="13 weeks ago → now",
        ),
        Patch(facecolor=_WARN, alpha=0.18, label=f"Crowded (|z| ≥ {POS_CROWDED_Z:g})"),
    ]
    leg = ax.legend(
        handles=handles,
        loc="upper center",
