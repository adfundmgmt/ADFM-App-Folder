...
from cte.adapters.base import read_cache, write_cache
...
def merge_cache(name: str, new_df: pd.DataFrame, keys: list[str]) -> pd.DataFrame:
    ...
    if existing is not None and {"ccy", "metric", "source"}.issubset(new_df.columns):
        ...
        in_touched = [tuple(x) in touched for x in combined[["ccy", "metric"]].values]
        stale = (
            pd.Series(in_touched, index=combined.index) & merged["_keep"].isna().values
        )
        if stale.any():
            dropped = combined[stale].groupby(["ccy", "metric", "source"]).size()
            print(
                f"[store] pruned {int(stale.sum())} stale-source rows in '{name}': "
                f"{dict(dropped)}"
            )
...
def depth_audit(
    df: pd.DataFrame, group_cols: list[str], date_col: str = "date"
) -> pd.DataFrame:
