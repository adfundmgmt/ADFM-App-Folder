...
def completeness_warnings(pillars: pd.DataFrame) -> dict[str, str]:
    ...
    for ccy in chk.index:
        missing = [
            PILLAR_DISPLAY.get(p, p) for p in expected if pd.isna(chk.loc[ccy, p])
        ]
        if missing:
            out[ccy] = (
                f"Incomplete data — missing the {', '.join(missing)} pillar(s); "
...
def context_notes(
    tm: pd.DataFrame, pillars: pd.DataFrame, real_grid: pd.DataFrame
) -> dict[str, list[str]]:
    q = tm.set_index("ccy")
    carry_avg = {
        c: real_grid.loc[c].drop(c, errors="ignore").mean() for c in real_grid.index
    }
    ...
    for c in q.index:
        ...
        if pd.notna(f) and pd.notna(v):
            if f > FUND_HI and v < STRETCH_LO:
                notes.append(
                    f"Cheap and improving — the mean-reversion setup: valuation "
                    f"is below its own norm ({v:+.1f}) while fundamentals are "
                    f"turning up ({f:+.1f})."
                )
            elif f < FUND_LO and v > STRETCH_HI:
                notes.append(
                    f"Expensive and deteriorating — the vulnerable combination: "
        ...
        if (
            ...
            base = (
                ...
                if v > CARRY_FRAGILE_STRETCH
                else "a deteriorating fundamental trajectory"
            )
            notes.append(
                f"Fat carry on a fragile base: out-yields the field in real "
        ...
        if c in pillars.index:
            ...
            if len(prow) >= 3:
                ...
                if abs(mag) > DOMINANCE_MIN and abs(mag) > DOMINANCE * rest:
                    notes.append(
                        f"One-legged read: {PILLAR_DISPLAY.get(dom, dom)} "
