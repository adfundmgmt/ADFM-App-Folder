...
def _parse_period(s: pd.Series) -> pd.Series:
    ...
    if is_q.any():
        out[is_q] = (
            pd.PeriodIndex(txt[is_q].str.replace("-", ""), freq="Q")
            .to_timestamp(how="end")
            .normalize()
        )
    if (~is_q).any():
        out[~is_q] = pd.to_datetime(txt[~is_q], format="%Y-%m") + pd.offsets.MonthEnd(0)
    return out
...
def fetch_oecd_all() -> pd.DataFrame:
    sess = make_session()
    parts = [
        fetch_oecd_bcicp(session=sess),
        fetch_oecd_cpi(session=sess),
        fetch_oecd_gdp(session=sess),
        fetch_oecd_unemployment(session=sess),
    ]
