...
def fetch_current_account(session=None) -> pd.DataFrame:
    ...
    url = (
        f"{OECD_SDMX_BASE}/{OECD_BOP_DATAFLOW}/all"
        f"?startPeriod=1990-Q1&dimensionAtObservation=AllDimensions"
    )
    r = sess.get(
        url,
        headers={"User-Agent": HTTP_UA, "Accept": OECD_CSV_ACCEPT},
...
def fetch_niip(session=None) -> pd.DataFrame:
    ...
    """
    from cte.config import OECD_IIP_DATAFLOW, OECD_QNA_DATAFLOW
    ...
    iip = iip[
        (iip.MEASURE == "FA")
        & (iip.ACCOUNTING_ENTRY == "N")
        & (iip.UNIT_MEASURE == "XDC")
        & (iip.FREQ == "Q")
        & (iip.FS_ENTRY == "LE")  # LE = closing stock (the NIIP itself);
        & (iip.REF_AREA.isin(a2c))
    ].copy()  # exclude K*/KA revaluation-flow entries
    ...
    gdp["ann_gdp"] = (
        gdp.groupby("ccy")["ngdp"].rolling(4).sum().reset_index(level=0, drop=True)
    )
...
def _fetch_eurostat_niip(sess) -> pd.DataFrame:
    ...
    raw = pd.read_csv(
        io.BytesIO(
            sess.get(url, headers={"User-Agent": HTTP_UA}, timeout=HTTP_TIMEOUT).content
        )
    )
    d = raw[raw.geo == "EA20"].copy()
    d["date"] = (
        pd.PeriodIndex(d.TIME_PERIOD.str.replace("-", ""), freq="Q")
