...
def build_features() -> pd.DataFrame:
    ...
    for ccy in mac["ccy"].unique():
        ...
        if "bcicp" in wide:
            emit(
                wide[["bcicp"]].rename(columns={"bcicp": "value"}).reset_index(),
                ccy,
                "bcicp",
            )
            emit(
                _chg(wide["bcicp"], 3).rename("value").reset_index(), ccy, "bcicp_slope"
            )
        if "gdp" in wide:
            emit(_pct(wide["gdp"], 12).rename("value").reset_index(), ccy, "gdp_yoy")
        if "unemp" in wide:
            emit(
                _chg(wide["unemp"], 3).rename("value").reset_index(),
                ccy,
                "unemp_3m_chg",
            )
        ...
        if "cpi" in wide:
            ...
            emit(
                (wide["cpi"].dropna() - tgt).rename("value").reset_index(),
                ccy,
                "infl_gap",
            )
            emit(
                _chg(wide["cpi"], 3).rename("value").reset_index(), ccy, "infl_momentum"
            )
        ...
        if len(yc):
            ...
            pol = None
            if "policy" in wide:
                pol = wide["policy"].copy()
                pol.index = _month_end(pol.index)
                pol = (
                    pol[~pol.index.duplicated(keep="last")].reindex(grid).ffill(limit=4)
                )

            # Pillar F: carry legs
            if "nominal_2y" in yc:
                emit(
                    yc["nominal_2y"].rename_axis("date").rename("value").reset_index(),
                    ccy,
                    "nominal_2y",
                )
                if cpi_al is not None:
                    ...
                    emit(
                        real2.rename_axis("date").rename("value").reset_index(),
                        ccy,
                        "real_2y",
                    )
                if pol is not None:
                    ...
                    emit(
                        pp.rename_axis("date").rename("value").reset_index(),
                        ccy,
                        "priced_path",
                    )
            ...
            if ccy == "USD":
                if "real_10y" in wide:
                    r10 = wide["real_10y"].copy()
                    r10.index = _month_end(r10.index)
                    r10 = r10[~r10.index.duplicated(keep="last")]
                    emit(
                        r10.rename_axis("date").rename("value").reset_index(),
                        ccy,
                        "real_10y",
                    )
            elif "nominal_10y" in yc and cpi_al is not None:
                emit(
                    (yc["nominal_10y"] - cpi_al)
                    .rename_axis("date")
                    .rename("value")
                    .reset_index(),
                    ccy,
            ...
            if pol is not None and cpi_al is not None:
                emit(
                    (pol - cpi_al).rename_axis("date").rename("value").reset_index(),
                    ccy,
                    "real_policy",
                )
...
if __name__ == "__main__":
    f = build_features()
    print(
        f"features: {len(f)} rows | {f.feature.nunique()} features | "
