...
def _parse_custom_events(text: str, warnings: list | None = None) -> pd.DataFrame:
    ...
    try:
        ...
        warnings.append(f"Could not parse custom CSV: {exc}")
        return pd.DataFrame(columns=cols)
    df.columns = [str(c).strip() for c in df.columns]
    if not {"Date", "Event"}.issubset(df.columns):
        warnings.append("Custom calendar requires Date and Event columns.")
        return pd.DataFrame(columns=cols)
...
def _official_dated_calendar(start: date, horizon_days: int, include_fed: bool) -> pd.DataFrame:
    ...
    recurring = _build_rule_calendar(start, horizon_days, include_fed=False)
    ...
    calendar["Type"] = calendar["Type"].map(_normalize_event_type)
